...
// Define the top left corner of player
int	xPlayerStart 	= 40; 
int	yPlayerStart	= 10;
int	PlayerWidth 	= 16;
int	PlayerHeight 	= 16;
...
void OrbitDemo0() { // does simple arithmetic calculations, reads operands and operators serially from computer
  OrbitOledClear();
  OrbitOledMoveTo(0,0);
  OrbitOledSetCursor(0,0);
  ...
  while (CheckSwitches() == DEMO_0) //only works for positive numbers and integer division (ie always rounds down if decimal) 
  {
    ...
    OrbitOledClear(); // print result to OLED
    OrbitOledMoveTo(0,0);
    OrbitOledSetCursor(0,0);
    if (result == 0){
      OrbitOledPutChar('0');
...
void OrbitDemo2() { // plays doodlejump on the OLED display using the accelerometer for tilt control
  char printVal[10];
...
void OrbitDemo3() {
  ...
  OrbitOledSetCursor(0,0);
  ClearLED = true;
  OrbitOledPutString("There is no demo 3.");
  while (CheckSwitches() == DEMO_3){}
}

// morse code patterns for a-z
// 1 morse code unit is half a second
void space(){
  delay(2000);
}
